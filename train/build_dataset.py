...
def build(input_dir, output_dir, layout="coco", annotation=None,
    ...
    if max_per_class:
        # Shuffle first so the cap draws a random cross-section of subjects
        # rather than just whichever files sort first alphabetically.
    ...
    for path in files:
        ...
        if remap is not None:
            ...
            cat = remap[cat]
        if max_per_class and per_class_count.get(cat, 0) >= max_per_class:
            continue
        per_class_count[cat] = per_class_count.get(cat, 0) + 1
...
def _cli():
    ...
    p.add_argument("--max-per-class", type=int,
                   help="cap clips kept per class (random cross-section), "
                        "for controlled dataset-size comparisons")
    args = p.parse_args()
